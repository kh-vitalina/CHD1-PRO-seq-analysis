...
ax.scatter(nonsig['log2FoldChange'], -np.log10(nonsig['padj']),
           s=3, alpha=0.25, color='#D0D0D0', edgecolors='none', rasterized=True)
ax.scatter(sig['log2FoldChange'], -np.log10(sig['padj']),
           s=3, alpha=0.45, color='#F4A582', edgecolors='none', rasterized=True)
ax.scatter(sig_up['log2FoldChange'], -np.log10(sig_up['padj']),
           s=6, alpha=0.7, color='#D73027', edgecolors='none', rasterized=True)
ax.scatter(sig_down['log2FoldChange'], -np.log10(sig_down['padj']),
           s=6, alpha=0.7, color='#4575B4', edgecolors='none', rasterized=True)

ax.axhline(-np.log10(0.05), ls='--', color='#999999', lw=0.5)
ax.axvline(-1, ls='--', color='#999999', lw=0.5)
ax.axvline(1, ls='--', color='#999999', lw=0.5)

ax.text(0.96, 0.96, f'↑ {len(sig_up)}', transform=ax.transAxes, fontsize=9,
        ha='right', va='top', color='#D73027', fontweight='bold')
ax.text(0.04, 0.96, f'↓ {len(sig_down)}', transform=ax.transAxes, fontsize=9,
        ha='left', va='top', color='#4575B4', fontweight='bold')
...
if HAS_ADJUSTTEXT:
    ...
    for _, row in top_label.iterrows():
        y_val = min(-np.log10(max(row['padj'], 1e-300)), ax.get_ylim()[1] - 5 if ax.get_ylim()[1] > 10 else 300)
        texts.append(ax.annotate(
            row[gene_col], (row['log2FoldChange'], y_val),
            fontsize=6.75, color='#333333', ha='center', va='bottom',
            xytext=(0, 3), textcoords='offset points'))
    adjust_text(texts, ax=ax, arrowprops=dict(arrowstyle='-', color='#999999', lw=0.5))
else:
    for _, row in top_label.iterrows():
        y_val = min(-np.log10(max(row['padj'], 1e-300)), 280)
        ax.annotate(row[gene_col], (row['log2FoldChange'], y_val),
...
im = ax.imshow(z_ordered.values, aspect='auto', cmap=cmap, vmin=-2.5, vmax=2.5,
               interpolation='none')

sample_labels = ['OsTir1\nCtrl 1', 'OsTir1\nCtrl 2', 'OsTir1\nAux 1', 'OsTir1\nAux 2',
                 'Chd1\nCtrl 1', 'Chd1\nCtrl 2', 'Chd1\nAux 1', 'Chd1\nAux 2']
...
if up_ordered and down_ordered:
    ...
    ax.text(-1.5, len(up_ordered) / 2, '↑', ha='center', va='center',
            fontsize=10, color='#BF616A', fontweight='bold', clip_on=False)
    ax.text(-1.5, len(up_ordered) + len(down_ordered) / 2, '↓',
            ha='center', va='center', fontsize=10, color='#5E81AC',
...
for i in range(8):
    for j in range(8):
        ...
        ax.text(j, i, f'{val:.3f}', ha='center', va='center',
                fontsize=5.5, color='white' if val < 0.93 else 'black')

# highlight condition blocks
for start, end, color in [(0, 2, '#5E81AC'), (2, 4, '#81A1C1'),
                           (4, 6, '#A3BE8C'), (6, 8, '#BF616A')]:
    rect = plt.Rectangle((start - 0.5, start - 0.5), end - start, end - start,
                          fill=False, edgecolor=color, linewidth=2, clip_on=False)
    ax.add_patch(rect)
