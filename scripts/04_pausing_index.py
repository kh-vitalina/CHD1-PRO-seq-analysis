...
for name, bam_path in SAMPLES.items():
    ...
    print(
        f"    n={len(sample_pi[name])}, median PI={sample_pi[name]['PI'].median():.2f}"
    )
...
for ax, (data, title) in zip(axes, [(chrX, "X chromosome"), (auto, "Autosomes")]):
    bp = ax.boxplot(
        [np.log2(data["PI_ctrl"]), np.log2(data["PI_mut"])],
...
fc_main = (
    conditions["Chd1_Auxin"]["PI"].median() / conditions["OsTir1_Auxin"]["PI"].median()
)
