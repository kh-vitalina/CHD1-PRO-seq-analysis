...
for chrom in chrom_order:
    ...
    _, p = (
        stats.wilcoxon(sub["rpk_ctrl"], sub["rpk_mut"])
        if len(sub) > 10
        else (None, 1.0)
    )
    chrom_stats[chrom] = {"n": len(sub), "median": sub["log2FC"].median(), "p": p}
    print(
        f"  {chrom}: n={len(sub)}, median log2FC={sub['log2FC'].median():.3f}, p={p:.2e}"
    )
...
bars = ax.bar(
    range(len(ratios)), ratios, color=bar_colors, alpha=0.7, edgecolor="black"
)
...
ax.bar(
    range(len(chrom_order)), medians, color=chrom_colors, alpha=0.7, edgecolor="black"
)
...
print("\nSummary:")
print(
    f"  X/A ratio: control={xa_ratios['OsTir1_Auxin']:.3f}, "
