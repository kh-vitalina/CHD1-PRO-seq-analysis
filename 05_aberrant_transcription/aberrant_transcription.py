...
def merge_replicates(df1, df2, label):
    ...
    merged['antisense_ratio'] = (merged['antisense_ratio_r1'] + merged['antisense_ratio_r2']) / 2
...
for ax, (c1, c2, title) in zip(axes, [
    ...
    m = conditions[c1][['gene', 'antisense_ratio']].merge(
        conditions[c2][['gene', 'antisense_ratio']], on='gene', suffixes=('_1', '_2'))
    log2fc = np.log2((m['antisense_ratio_2'] + 0.001) / (m['antisense_ratio_1'] + 0.001))
...
m = conditions['OsTir1_Auxin'][['gene', 'antisense_ratio']].merge(
    ...
    on='gene', suffixes=('_ctrl', '_mut'))
fig, ax = plt.subplots(figsize=(7, 7))
ax.scatter(np.log2(m['antisense_ratio_ctrl'] + 0.001),
...
merged = conditions['OsTir1_Auxin'][['gene', 'chrom', 'antisense_ratio']].merge(
    conditions['Chd1_Auxin'][['gene', 'antisense_ratio']],
    on='gene', suffixes=('_ctrl', '_mut'))
merged['log2FC'] = np.log2((merged['antisense_ratio_mut'] + 0.001) /
                            (merged['antisense_ratio_ctrl'] + 0.001))
...
for _, row in top20.iterrows():
    print(f"{row['gene']:<15} {row['chrom']:<8} "
...
at_labels = ['OsTir1\nCtrl', 'OsTir1\n+ Auxin', 'Chd1-AID\nCtrl', 'Chd1-AID\n+ Auxin']
data_styled = [np.log2(conditions[c]['antisense_ratio'].values + 0.001) for c in cond_order]
...
bp = ax.boxplot(data_styled, positions=[1, 2, 3, 4], patch_artist=True,
...
for i, d in enumerate(data_styled):
    med = np.median(d)
    ax.text(i + 1, med - 0.35, f'{med:.2f}', ha='center', va='top',
            fontsize=6.5, color='#555555', style='italic')

# stats for brackets
m_main = conditions['OsTir1_Auxin'][['gene', 'antisense_ratio']].merge(
    conditions['Chd1_Auxin'][['gene', 'antisense_ratio']], on='gene', suffixes=('_1', '_2'))
_, p_main = stats.wilcoxon(m_main['antisense_ratio_1'], m_main['antisense_ratio_2'])
fc_at = conditions['Chd1_Auxin']['antisense_ratio'].median() / conditions['OsTir1_Auxin']['antisense_ratio'].median()

y_top = max(np.percentile(d, 75) + 1.5 * (np.percentile(d, 75) - np.percentile(d, 25))
            for d in data_styled)

# main bracket
ax.plot([2, 2, 4, 4], [y_top + 0.4, y_top + 0.52, y_top + 0.52, y_top + 0.4], lw=0.8, c='#333333')
ax.text(3, y_top + 0.57, f'***  FC={fc_at:.1f}x', ha='center', va='bottom', fontsize=7.5, color='#333333')

# control bracket
ax.plot([1, 1, 2, 2], [y_top - 0.1, y_top, y_top, y_top - 0.1], lw=0.8, c='#333333')
ax.text(1.5, y_top + 0.03, 'n.s.', ha='center', va='bottom', fontsize=7.5, color='#888888')

# background
ax.axvspan(0.5, 2.5, alpha=0.08, color='#5E81AC', zorder=0)
ax.axvspan(2.5, 4.5, alpha=0.08, color='#BF616A', zorder=0)
ax.text(1.5, ax.get_ylim()[0] + 0.15, 'Controls', ha='center', fontsize=7,
        color='#5E81AC', fontweight='bold', alpha=0.7)
ax.text(3.5, ax.get_ylim()[0] + 0.15, 'Chd1-AID', ha='center', fontsize=7,
        color='#BF616A', fontweight='bold', alpha=0.7)
